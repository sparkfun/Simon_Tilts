...
// Define an array of "good" readings from the tilt sensors
  // Notice how 1 and 6 are not included as possibilities in the table above.
  // This is because of the particular angle and placement of the tilt arms.
  // 1 and 6 are impossible, so in an effort to debounce the data, we create the 
...
void listen_for_pattern()
{
  for(int i = 1; i<=game_length;i++)
  {
    ...
    while(timeout<50)
    {
      ...
      // (1) Are they on the correct next position?
      if(current_tilt_position == game_pattern[i]) 
        {
          // Blink current position once to indicate they did it correctly
          blink_led_w_fade(current_tilt_position); 
          break;
        }
      ...
      else if(current_tilt_position == game_pattern[i-1]); 
      
      // (4) Did the player rotate to the wrong position? 
      //  If so, keep track of how long by incrementing "wrong_positions"
      else if(current_tilt_position != game_pattern[i]) wrong_positions += 1; 
      ...
      if(wrong_positions > 15) 
      {
        ...
        blink_led(current_tilt_position, 50); 
        blink_all(50);
        // Blink where the player should have rolled to
        blink_led(game_pattern[i], 100); 
        delay(10);
        // Blink where the player should have rolled to (again)
        blink_led(game_pattern[i], 100); 
        delay(10);
        // Blink where the player should have rolled to (and  a 3rd time)
        blink_led(game_pattern[i], 400); 
        game_length = 0; // this gets us out of listen_for_pattern()
        level = 1;
        fail = 1;
        waiting_mode = true;
...
void blink_led_w_fade(int num)
{
  ...
  {
    analogWrite(led_gnd_pin, fadeValue); // sets the value (range from 0-255):  
...
// Define a function to indicate the the player has timed out of gameplay
  ...
  // It is intended to be very different from winner_blinky() which is kind of 
  // "cheery"
...
// Define a function that will show various blinky action and sound buzzing
  // This is used to control what the player sees and hears when they win each 
  // level. It's basically a way to call winner_blinky different amounts of 
  // times at different speeds. This way when you win each level, you see a 
  // unique display and it sort of shows you which level you just completed.
void show_winner_blinkies()
{
  ...
  delay(1000); // Give the player a second to get ready for the next level
}


// Define a function to blink the six LEDs in sequence, while also playing tones
  // Send this function an integer, increment_val, to speed up or slow down the 
  // sequence. This function is basically comprised of two for loops.
  // One to blink LEDs and buzz tones that start at low pitches and then climb 
  // up to high pitches. And a second to start at high pitches and then decline 
  // down to low pitches. It's mostly just a fun thing to see and hear when you 
  // complete a level.
...
// Define a function to show current game pattern to user
  // This function blinks a sequence of LEDs (and also buzzes each corresponding 
  // tone). It is basically a for loop that starts at the beginning, and then 
  // continues through the entire game length. Note, gamelength is incremented 
  // in the main loop. As the player repeats the current "sub" pattern correctly, 
  // gamelength increments. This continues on until they win the current level.
...
// Define a function to set level variables
  ...
  // This gameplay variables are then used in listen_for_pattern() to control 
...
// Define a function to be used during "waiting mode"
  // This function basically blinks al six LEDs and waits for the player to 
  // tilt the game in any direction. It reads the current position, and then 
  // reads a second time. And then compares the two readings to know if it's 
  // changed. When this happens, it changes the global variable, "waiting_mode" 
  // to false, which ultimately kills waiting mode.
void wait_for_user_input_change(void)
{
  // store the current position so we can compare it later and notice a change.
  byte prev_pos = read_position_once(); 
  blink_all(50); // Blink to indicate we're in waiting mode
  // if the player has rotated, then this new "fresh" reading will be different 
  // than "prev_pos"
  if(prev_pos != read_position_once()) 
  {
    delay(1000); // Delay to allow the user to return to the home position 
                 //(with the silk "Simon Tilts" reading left to right)
    waiting_mode = false; // This ultimately will get us out of the waiting mode 
    // while loop (at the top of the main loop) and then move on to the gameplay 
    // (which lives in the remainder of the main loop)
  }
}
...
// Define a function to update the next possible positions
  ...
  // This function acts as a set of "rules" to ensure we don't have two of the 
  // same positions in a row for any game pattern. You send it the current 
  // position, and it will change the values in the array 
  // next_possible_positions[]. The new values will only include positions 
  // that are adjacent to the current position.
...
void nightlight_fade(int num){
  digitalWrite(led_gnd_pin, HIGH); // Turn the GND return HIGH, 
                                   // which effectively turns all LEDs off
  ...
  // we start at 255 (HIGH - or off).
  // And then slowly decrease to 0 (LOW - full brightness)
  ...
  // Fade OUT, note because we are using PWM on the GND return to control the 
  // fade, we start at 0 (LOW) and then slowly increase to 255 (HIGH)
